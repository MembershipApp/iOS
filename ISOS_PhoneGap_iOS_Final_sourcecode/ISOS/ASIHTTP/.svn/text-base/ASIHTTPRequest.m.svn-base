...
- (void)appendPostData:(NSData *)data
{
    ...
    NSLog(@"%@",[[NSString alloc]initWithData:postBody encoding:NSStringEncodingConversionAllowLossy]);
	...
	if ([self temporaryFileDownloadPath]) {
		...
		if ([self shouldRedirect] && [self needsRedirect] && [self allowResumeForFileDownloads]) {
			...
			if (!fileError) {
				...
				if (moveError) {
                    NSLog(@"%d", [moveError code]);

                    NSString *tempPath = [NSString stringWithString:[self downloadDestinationPath]];
                    NSString *finalPath = [tempPath stringByDeletingLastPathComponent];
                    
                    [[NSFileManager defaultManager] createDirectoryAtPath:finalPath withIntermediateDirectories:YES
                                                               attributes:nil
                                                                    error:nil];
                    moveError = nil;
